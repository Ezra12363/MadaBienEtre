...
async def predict_acceptance_probability(
    ...
    if booking.therapist_id:
        ...
        if therapist and therapist.last_location and booking.client_location:
            distance = 0  # Calculer la distance
            # Si distance < 5km
            probability += 0.1
        else:
            probability += 0.05
    ...
    if booking.created_at:
        ...
        if 9 <= hour <= 17:
            probability += 0.05  # Heures de bureau
    ...
    if booking.therapist_id:
        # Taux d'acceptation du thérapeute
    ...
    if booking.client_id:
        ...
        if analytics:
            # Client fidèle
            if analytics.total_bookings > 5:
                probability += 0.05
    
    # 7. Facteur: Saisonnalité
    if booking.scheduled_date:
        month = booking.scheduled_date.month
        if month in [6, 7, 8, 12]:  # Saison haute
            probability -= 0.05
        elif month in [1, 2, 3]:  # Saison basse
            probability += 0.05
...
async def predict_demand(
    ...
    # Compter les réservations passées
