...
async def detect_fraud(booking: Booking, db: Session) -> Dict[str, Any]:
    ...
    if client:
        ...
        # Pas d'historique de réservation
    ...
    if booking.status in ["cancelled_by_client", "cancelled_by_therapist"]:
        # Nombre d'annulations du client
        client_cancellations = db.query(Booking).filter(
            Booking.client_id == booking.client_id,
            Booking.status.in_(["cancelled_by_client", "cancelled_by_therapist"])
        ).count()
        
        if client_cancellations > 5:
            flags.append("Taux d'annulation élevé")
            risk_score += 0.2
    
    # 6. Vérifier les avis suspects
    if booking.status == "completed":
        from ..models.review import Review
        review = db.query(Review).filter(Review.booking_id == booking.id).first()
        if review and review.rating < 2:
            # Note très basse
            flags.append("Note très basse")
            risk_score += 0.1
    
    # 7. Vérifier les adresses
    if booking.address:
        # Adresse suspecte (simplifié)
        suspicious_keywords = ["test", "fake", "unknown", "inconnu"]
        if any(keyword in booking.address.lower() for keyword in suspicious_keywords):
            flags.append("Adresse suspecte")
            risk_score += 0.1
